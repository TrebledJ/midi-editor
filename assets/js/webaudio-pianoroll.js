...
// <div id="wac-menu">abc</div>
            ...
            this.play = function (actx, playcallback, tick) {
                function Interval() {
                    ...
                    while (
                        this.timestack.length > 1 &&
                        current >= this.timestack[1][0]
                    ) {
                        this.timestack.shift();
                    }
                    this.cursor =
                        this.timestack[0][1] +
                        (current - this.timestack[0][0]) / this.timestack[0][2];
                    this.redrawMarker();
                    while (current + this.preload >= this.time1) {
                        ...
                        if (!e || e.t >= this.markend) {
                            ...
                            if (
                                this.editmode == "gridmono" ||
                                this.editmode == "gridpoly"
                            )
                            ...
                            if (!e || e.t >= this.markend) {
                                ...
                                this.time1 +=
                                    (e.t - this.tick1) * this.tick2time;
                        }
                    }
                }
                if (typeof tick != "undefined") this.locate(tick);
            ...
            this.hitTest = function (pos) {
                ...
                ht.t =
                    this.xoffset +
                    ((pos.x - this.yruler - this.kbwidth) / this.swidth) *
                        this.xrange;
                ...
                for (let i = 0; i < l; ++i) {
                    ...
                    if ((ht.n | 0) == ev.n) {
                        ...
                        if (
                            ev.f &&
                            Math.abs(ev.t + ev.g - ht.t) * this.stepw < 8
                        ) {
                            ht.m = "E";
                            ht.i = i;
                            return ht;
            ...
            this.delSelectedNote = function () {
                ...
                for (let i = l - 1; i >= 0; --i) {
                    const ev = this.sequence[i];
                    if (ev.f) this.sequence.splice(i, 1);
                }
            };
            this.moveSelectedNote = function (dt, dn) {
                ...
                for (let i = 0; i < l; ++i) {
                    const ev = this.sequence[i];
                    if (ev.f && ev.ot + dt < 0) dt = -ev.ot;
                }
                for (let i = 0; i < l; ++i) {
                    const ev = this.sequence[i];
                    if (ev.f) {
                        ev.t =
                            (((ev.ot + dt) / this.snap + 0.5) | 0) * this.snap;
                        ev.n = ev.on + dn;
                    }
                }
            };
            ...
            this.selectedNotes = function () {
                ...
                for (let i = this.sequence.length - 1; i >= 0; --i) {
                    const ev = this.sequence[i];
                    if (ev.f) obj.push({ i: i, ev: ev, t: ev.t, g: ev.g });
                }
                return obj;
            };
                        ...
                        ev: [
                            {
                                t: t,
                                g: 1,
                                ev: this.sequence[this.sequence.length - 1],
                            },
                        ],
                    };
                    this.redraw();
                ...
                if (this.dragging.o == "D") {
                    switch (this.dragging.m) {
                        ...
                        case "B":
                            ...
                            t =
                                ((Math.max(0, ht.t) / this.snap + 0.5) | 0) *
                                this.snap;
                            ev.g = ev.t + ev.g - t;
                            ev.t = t;
                            if (ev.g < 0) {
                        ...
                        case "N":
                            ...
                            this.moveSelectedNote(
                                (ht.t - this.dragging.t) | 0,
                                (ht.n | 0) - this.dragging.n
                            );
                            this.redraw();
                            break;
                    }
            ...
            this.setListener = function (el, mode) {
                ...
                if (mode) {
                    el.addEventListener(
                        "mouseover",
                        this.pointerover.bind(this),
                        false
                    );
                    el.addEventListener(
                        "mouseout",
                        this.pointerout.bind(this),
                        false
                    );
                }
            };
            ...
            this.getPos = function (e) {
                ...
                if (
                    this.lastx >= this.rcMenu.x &&
                    this.lastx < this.rcMenu.x + this.rcMenu.width &&
                    this.lasty >= this.rcMenu.y &&
                    this.lasty < this.rcMenu.y + this.rcMenu.height
                )
                    t = this.menu;
            ...
            this.pointerdown = function (ev) {
                ...
                this.longtapcount = 0;
                this.longtaptimer = setInterval(
                    this.longtapcountup.bind(this),
                    100
                );
                ...
                if (e.button == 2 || e.ctrlKey) {
                    switch (this.downht.m) {
                        ...
                        default:
                            if (
                                this.editmode == "dragmono" ||
                                this.editmode == "dragpoly"
                            )
                                this.dragging = {
                                    o: "A",
                                    p: this.downpos,
            ...
            this.pointermove = function (ev) {
                ...
                if (this.longtaptimer) clearInterval(this.longtaptimer);
                const pos = this.getPos(e);
                const ht = this.hitTest(pos);
                switch (this.dragging.o) {
                    ...
                    case "E":
                        var p = Math.max(
                            1,
                            (this.dragging.m +
                                (pos.x - this.dragging.x) / this.stepw +
                                0.5) |
                                0
                        );
                        if (this.markstart >= p) this.markstart = p - 1;
                        this.markend = p;
                        break;
                    case "S":
                        var p = Math.max(
                        ...
                        if (this.markend <= p) this.markend = p + 1;
                        this.markstart = p;
                        break;
                    case "P":
            ...
            this.cancel = function (ev) {
                ...
                if (this.dragging.o == "m") {
                    ...
                    this.rcMenu = { x: 0, y: 0, width: 0, height: 0 };
                    if (pos.t == this.menu) this.delSelectedNote();
                    this.redraw();
                }
            ...
            this.redrawMarker = function () {
                ...
                const start =
                    ...
                    this.yruler +
                    this.kbwidth;
                this.markstartimg.style.left =
                    start + this.markstartoffset + "px";
                const end =
                    (this.markend - this.xoffset) * this.stepw +
                    this.yruler +
                    this.kbwidth;
            ...
            this.redrawGrid = function () {
                for (let y = 0; y < 128; ++y) {
                    ...
                    else this.ctx.fillStyle = this.collt;
                    let ys = this.height - (y - this.yoffset) * this.steph;
                    this.ctx.fillRect(
                        this.yruler + this.kbwidth,
                        ys | 0,
                        this.swidth,
                        -this.steph
                    );
            ...
            this.redrawYRuler = function () {
                if (this.yruler) {
                    ...
                    for (let y = 0; y < 128; y += 12) {
                        const ys =
                            this.height - this.steph * (y - this.yoffset);
                        this.ctx.fillRect(0, ys | 0, this.yruler, -1);
                        this.ctx.fillText(
                            "C" + (((y / 12) | 0) + this.octadj),
                            this.yruler - 4,
                            ys - 4
                        );
                    }
            ...
            this.redrawAreaSel = function () {
                if (this.dragging && this.dragging.o == "A") {
                    ...
                    this.ctx.fillRect(
                        this.dragging.p.x,
                        this.dragging.p.y,
                        this.dragging.p2.x - this.dragging.p.x,
                        this.dragging.p2.y - this.dragging.p.y
                    );
                }
            };
            this.redraw = function () {
                ...
                for (let s = 0; s < l; ++s) {
                    ...
                    if (ev.f) this.ctx.fillStyle = this.colnoteselborder;
                    else this.ctx.fillStyle = this.colnoteborder;
